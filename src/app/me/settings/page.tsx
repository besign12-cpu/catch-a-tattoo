...
export default async function SettingsPage() {
  ...
  if (!user) {
    redirect("/auth/login?next=/me/settings");
  }

  // users 테이블에서 Base City + base_city_changed_at 조회
  const { data: userData } = await supabase
    .from("users")
    .select("base_city, base_country, base_city_changed_at")
    .eq("id", user.id)
    .single();
